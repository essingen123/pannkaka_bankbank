...
def install_pannkaka_button():
    ...
    shortcuts = [
        {
            "name": "Pannkaka Button Toggle",
            "command": f"gnome-terminal -- bash -c '{script_path}'",
            "key": TOGGLE_KEY,
            "index": 0,
        },
        {
            "name": "Pannkaka Button Deactivate",
            "command": f"gnome-terminal -- bash -c 'rm /tmp/pannkaka_mode && {script_path}'",
            "key": DEACTIVATE_KEY,
            "index": 1,
        },
        {
            "name": "Pannkaka Button Activate",
            "command": f"gnome-terminal -- bash -c '{script_path}'",
            "key": ACTIVATE_KEY,
            "index": 2,
        },
    ...
    for shortcut in shortcuts:
        ...
        run_command(f"dconf write {path}/name \"'{shortcut['name']}'\"")
        run_command(f"dconf write {path}/command \"'{shortcut['command']}'\"")
        run_command(
            f"dconf write {path}/binding {shortcut['key']}"
        )  # Use your escaping
